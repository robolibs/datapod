...
fn dynamic_vector_from_vec_and_push_pop_work() {
    let mut vector = mat::DVector::from_vec(vec![1, 2, 3]);
    vector.push(4);
...
fn dynamic_vector_at_and_swap_work() {
    let mut left = mat::DVector::from_vec(vec![1, 2, 3]);
    let mut right = mat::DVector::from_vec(vec![4, 5]);
...
fn dynamic_matrix_empty_and_fill_work() {
    let mut matrix = mat::DMatrix::<i32>::new(0, 3);
    assert!(matrix.empty());
    matrix.resize(2, 2);
    matrix.fill(7);
...
fn dynamic_matrix_at_mut_and_iter_work() {
    let mut matrix = mat::DMatrix::<i32>::new(2, 2);
...
fn dynamic_matrix_swap_exchanges_shapes_and_values() {
    let mut left = mat::DMatrix::from_vec(1, 2, vec![1, 2]).unwrap();
    let mut right = mat::DMatrix::from_vec(2, 1, vec![3, 4]).unwrap();
    left.swap(&mut right);
    assert_eq!(
...
fn dynamic_tensor_new_builds_expected_default_storage() {
    let tensor = mat::DTensor::<i32>::new(vec![2, 2, 2]);
    assert_eq!(tensor.as_slice(), &[0; 8]);
}

#[test]
fn dynamic_tensor_dim_and_empty_work() {
    let tensor = mat::DTensor::<i32>::default();
...
fn dynamic_tensor_fill_and_swap_work() {
    let mut left = mat::DTensor::<i32>::new(vec![2, 2, 1]);
    let mut right = mat::DTensor::<i32>::new(vec![1, 1, 1]);
    left.fill(3);
    right.fill(9);
    left.swap(&mut right);
...
fn dynamic_tensor_at_and_rank_validation_errors_work() {
    let tensor =
        mat::DTensor::<i32>::from_vec(vec![2, 2, 2], vec![1, 2, 3, 4, 5, 6, 7, 8]).unwrap();
...
fn dynamic_tensor_alias_is_available() {
    let tensor: mat::DynamicTensor<i32> = mat::DTensor::new(vec![1, 2, 3]);
