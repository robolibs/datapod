...
namespace bitcon {
    ...
    struct DynamicFwsMultimapBase {
        ...
        static constexpr auto const MAX_ENTRIES_PER_BUCKET = static_cast<size_type>(1ULL << Log2MaxEntriesPerBucket);
        ...
        template <bool Const> struct bucket {
            friend DynamicFwsMultimapBase;
...
#endif
            template <bool IsConst = Const, typename = std::enable_if_t<!IsConst>> operator bucket<true>() {
...
#pragma clang diagnostic pop
            ...
            const_iterator begin() const { return multimap_.data_.begin() + get_index().begin_; }
            ...
            value_type &operator[](size_type index) { return mutable_mm().data_[data_index(index)]; }

            value_type const &operator[](size_type index) const { return multimap_.data_[data_index(index)]; }

            value_type &at(size_type const index) { return mutable_mm().data_[get_and_check_data_index(index)]; }
            ...
            size_type bucket_index(const_iterator it) const {
                if (it < begin() || it >= end()) {
                    throw std::out_of_range{"dynamic_fws_multimap::bucket::bucket_index() out of range"};
                }
            ...
            template <bool IsConst = Const, typename = std::enable_if_t<!IsConst>, typename... Args>
            size_type emplace_back(Args &&...args) {
                return mutable_mm().emplace_back_entry(index_, std::forward<Args>(args)...);
            }
            ...
            template <bool IsConst = Const, typename = std::enable_if_t<!IsConst>>
            void resize(size_type const new_size, value_type const init = value_type{}) {
            ...
            template <bool IsConst = Const, typename = std::enable_if_t<!IsConst>> void pop_back() {
                if (!empty()) {
                    resize(static_cast<size_type>(size() - 1U));
                }
            }

            template <bool IsConst = Const, typename = std::enable_if_t<!IsConst>> void clear() {
                auto &index = get_index();
                auto &data = mutable_mm().data_;
            ...
            template <bool IsConst = Const, typename = std::enable_if_t<!IsConst>> iterator erase(iterator pos) {
          ...
          protected:
            bucket(DynamicFwsMultimapBase const &multimap, size_type const index)
            ...
            size_type get_and_check_data_index(size_type index) const {
                ...
                if (index >= idx.size_) {
                    throw std::out_of_range{"dynamic_fws_multimap::bucket::at() out of range"};
                }
                return idx.begin_ + index;
            }

            template <bool IsConst = Const, typename = std::enable_if_t<!IsConst>>
            iterator prepare_insert(typename bucket::iterator it) {
            ...
            DynamicFwsMultimapBase &mutable_mm() noexcept { return const_cast<DynamicFwsMultimapBase &>(multimap_); }

            DynamicFwsMultimapBase const &multimap_;
            size_type index_;
        };
        ...
        template <bool Const> struct bucket_iterator {
            friend DynamicFwsMultimapBase;
            ...
            template <bool IsConst = Const, typename = std::enable_if_t<IsConst>>
            bucket_iterator(bucket_iterator<false> const &it) : multimap_{it.multimap_}, index_{it.index_} {}

            value_type operator*() const {
                return const_cast<DynamicFwsMultimapBase &>(multimap_).at(access_t{index_});
            }

            value_type operator->() const { return multimap_.at(access_t{index_}); }

            template <bool IsConst = Const, typename = std::enable_if_t<!IsConst>> value_type operator->() {
                return const_cast<DynamicFwsMultimapBase &>(multimap_).at(access_t{index_});
            }
            ...
            bucket_iterator operator+(difference_type n) const { return {multimap_, index_ + n}; }
            ...
            value_type operator[](difference_type n) const { return multimap_.at(access_t{index_ + n}); }

            template <bool IsConst = Const, typename = std::enable_if_t<!IsConst>>
            value_type operator[](difference_type const n) {
                return const_cast<DynamicFwsMultimapBase &>(multimap_).at(access_t{index_ + n});
            }

            bool operator<(bucket_iterator const &rhs) const { return index_ < rhs.index_; }
            bool operator<=(bucket_iterator const &rhs) const { return index_ <= rhs.index_; }
            bool operator>(bucket_iterator const &rhs) const { return index_ > rhs.index_; }
            bool operator>=(bucket_iterator const &rhs) const { return index_ >= rhs.index_; }
          ...
          protected:
            bucket_iterator(DynamicFwsMultimapBase const &multimap, size_type const index)
                : multimap_{multimap}, index_{index} {}
        ...
        mutable_bucket get_or_create(access_t const index) {
            verify(index != std::numeric_limits<size_type>::max(), "mutable_fws_multimap::get_or_create: type bound");
        ...
        constexpr size_type max_entries_per_bucket() const noexcept { return MAX_ENTRIES_PER_BUCKET; }

        constexpr size_type max_entries_per_bucket_log2() const noexcept { return Log2MaxEntriesPerBucket; }

        iterator begin() { return {*this, size_type{0U}}; }
        const_iterator begin() const { return {*this, size_type{0U}}; }
        iterator end() { return iterator{*this, static_cast<size_type>(index_.size())}; }
        const_iterator end() const { return const_iterator{*this, static_cast<size_type>(index_.size())}; }

        friend iterator begin(DynamicFwsMultimapBase &m) { return m.begin(); }
        friend const_iterator begin(DynamicFwsMultimapBase const &m) { return m.begin(); }

        friend iterator end(DynamicFwsMultimapBase &m) { return m.end(); }
        friend const_iterator end(DynamicFwsMultimapBase const &m) { return m.end(); }
        ...
        void grow_bucket(size_type const map_index, index_type &idx, size_type const requested_capacity) {
            static_assert(std::is_trivially_copyable_v<T>);

            assert(requested_capacity > 0U);
            auto const new_capacity = size_type{next_power_of_two(to_idx(requested_capacity))};
            auto const new_order = get_order(new_capacity);

            verify(new_order <= Log2MaxEntriesPerBucket, "dynamic_fws_multimap: too many entries in a bucket");
            ...
            if (free_bucket) {
                if (old_bucket.capacity_ != 0U) {
                    move_entries(map_index, old_bucket.begin_, free_bucket->begin_, idx.size_);
        ...
        std::optional<index_type> get_free_bucket(size_type const requested_order) {
            ...
            return pop(free_buckets_[to_idx(requested_order)]);
        }
        ...
        void release_bucket(index_type &bucket) {
            if (bucket.capacity_ != 0U) {
                ...
                free_buckets_[to_idx(order)].push_back(index_type{bucket});
        ...
        void move_entries(size_type const /* map_index */, size_type const old_data_index,
            ...
            if constexpr (std::is_trivially_copyable_v<value_type>) {
                ...
                for (auto i = static_cast<size_type>(0); i < count; ++i, ++old_data, ++new_data) {
        ...
        template <typename... Args> size_type emplace_back_entry(size_type const i, Args &&...args) {
            auto const map_index = to_idx(i);
            auto const data_index = insert_new_entry(map_index);
