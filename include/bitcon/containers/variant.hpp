...
namespace bitcon {
    ...
    constexpr bitcon::type_at_index_t<I, Ts...> const &get(bitcon::Variant<Ts...> const &v) noexcept {
        return v.template as<bitcon::type_at_index_t<I, Ts...>>();
    }

    template <std::size_t I, typename... Ts>
    constexpr bitcon::type_at_index_t<I, Ts...> &get(bitcon::Variant<Ts...> &v) noexcept {
        return v.template as<bitcon::type_at_index_t<I, Ts...>>();
    }

    template <class T, class... Ts> constexpr T const &get(bitcon::Variant<Ts...> const &v) noexcept {
        static_assert(bitcon::index_of_type<T, Ts...>() != bitcon::TYPE_NOT_FOUND);
        return v.template as<T>();
    }

    template <class T, class... Ts> constexpr T &get(bitcon::Variant<Ts...> &v) noexcept {
        static_assert(bitcon::index_of_type<T, Ts...>() != bitcon::TYPE_NOT_FOUND);
        return v.template as<T>();
    }

    template <class T, class... Ts> constexpr std::add_pointer_t<T> get_if(bitcon::Variant<Ts...> &v) noexcept {
        static_assert(bitcon::index_of_type<T, Ts...>() != bitcon::TYPE_NOT_FOUND);
        return v.idx_ == bitcon::index_of_type<T, Ts...>() ? &v.template as<T>() : nullptr;
    }

    template <std::size_t I, typename... Ts>
    constexpr std::add_pointer_t<bitcon::type_at_index_t<I, Ts...> const>
    get_if(bitcon::Variant<Ts...> const &v) noexcept {
        static_assert(I < sizeof...(Ts));
        return v.idx_ == I ? &v.template as<bitcon::type_at_index_t<I, Ts...>>() : nullptr;
    }

    template <std::size_t I, typename... Ts>
    constexpr std::add_pointer_t<bitcon::type_at_index_t<I, Ts...>> get_if(bitcon::Variant<Ts...> &v) noexcept {
        static_assert(I < sizeof...(Ts));
        return v.idx_ == I ? &v.template as<bitcon::type_at_index_t<I, Ts...>>() : nullptr;
    }

    template <class T, class... Ts>
    constexpr std::add_pointer_t<T const> get_if(bitcon::Variant<Ts...> const &v) noexcept {
        static_assert(bitcon::index_of_type<T, Ts...>() != bitcon::TYPE_NOT_FOUND);
        return v.idx_ == bitcon::index_of_type<T, Ts...>() ? &v.template as<T>() : nullptr;
    }
...
namespace std {
    ...
    template <typename Visitor, typename... T> constexpr auto visit(Visitor &&vis, bitcon::Variant<T...> &&v) {
        return v.apply(std::forward<Visitor>(vis));
    }

    template <typename Visitor, typename... T> constexpr auto visit(Visitor &&vis, bitcon::Variant<T...> const &v) {
        return v.apply(vis);
    }
