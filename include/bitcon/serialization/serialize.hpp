...
namespace bitcon {
    ...
    template <typename T> struct is_bitcon_container : std::false_type {};
    template <> struct is_bitcon_container<String> : std::true_type {};
    template <typename T> struct is_bitcon_container<Vector<T>> : std::true_type {};
    template <typename T> struct is_bitcon_container<Optional<T>> : std::true_type {};
    template <typename A, typename B> struct is_bitcon_container<Pair<A, B>> : std::true_type {};
    template <typename T, std::size_t N> struct is_bitcon_container<Array<T, N>> : std::true_type {};
    template <typename... Ts> struct is_bitcon_container<Tuple<Ts...>> : std::true_type {};
    template <typename... Ts> struct is_bitcon_container<Variant<Ts...>> : std::true_type {};
    ...
    template <typename T, template <typename> typename Ptr, typename GetKey, typename GetValue, typename Hash,
              typename Eq>
    struct is_bitcon_container<HashStorage<T, Ptr, GetKey, GetValue, Hash, Eq>> : std::true_type {};
    template <typename T> constexpr bool is_bitcon_container_v = is_bitcon_container<decay_t<T>>::value;

    // Serialize aggregate types (structs) using reflection
    template <Mode M, typename Ctx, typename T>
    std::enable_if_t<std::is_class_v<T> && !std::is_scalar_v<T> && !is_bitcon_container_v<T>> //
