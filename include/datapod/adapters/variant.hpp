...
namespace datapod {
    ...
    template <class T, class... Ts> constexpr T const &get(datapod::Variant<Ts...> const &v) noexcept {
        static_assert(datapod::index_of_type<T, Ts...>() != datapod::TYPE_NOT_FOUND);
        return v.template as<T>();
    }

    template <class T, class... Ts> constexpr T &get(datapod::Variant<Ts...> &v) noexcept {
        static_assert(datapod::index_of_type<T, Ts...>() != datapod::TYPE_NOT_FOUND);
        return v.template as<T>();
    }

    template <class T, class... Ts> constexpr std::add_pointer_t<T> get_if(datapod::Variant<Ts...> &v) noexcept {
        static_assert(datapod::index_of_type<T, Ts...>() != datapod::TYPE_NOT_FOUND);
    ...
    constexpr std::add_pointer_t<T const> get_if(datapod::Variant<Ts...> const &v) noexcept {
        static_assert(datapod::index_of_type<T, Ts...>() != datapod::TYPE_NOT_FOUND);
        return v.idx_ == datapod::index_of_type<T, Ts...>() ? &v.template as<T>() : nullptr;
    }
...
namespace std {
    ...
    template <typename Visitor, typename... T> constexpr auto visit(Visitor &&vis, datapod::Variant<T...> &&v) {
        return v.apply(std::forward<Visitor>(vis));
    }

    template <typename Visitor, typename... T> constexpr auto visit(Visitor &&vis, datapod::Variant<T...> const &v) {
        return v.apply(vis);
    }
