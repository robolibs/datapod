...
namespace datapod {
    ...
    template <typename T, datapod::usize N = 0, typename = void> struct ArityImpl {
        static constexpr datapod::usize value = 0;
    };

    template <typename T, datapod::usize N>
    struct ArityImpl<T, N, std::enable_if_t<std::is_aggregate_v<T> && !std::is_array_v<T>>> {
      ...
      public:
        static constexpr datapod::usize value =
    ...
    template <typename T> struct ArityImpl<T, 128, std::enable_if_t<std::is_aggregate_v<T> && !std::is_array_v<T>>> {
        static constexpr datapod::usize value = 128; // Max supported arity
    };

    template <typename T> struct Arity {
        static constexpr datapod::usize value = ArityImpl<T>::value;
    };

    template <typename T> inline constexpr datapod::usize arity_v = Arity<T>::value;
