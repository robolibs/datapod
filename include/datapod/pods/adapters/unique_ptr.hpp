...
namespace datapod {
    ...
    template <typename T, typename Ptr> auto operator<=>(UniquePtr<T, Ptr> const &lhs, UniquePtr<T, Ptr> const &rhs) {
        return lhs.get() <=> rhs.get();
    }
#endif

    // Array specialization for UniquePtr<T[]>
    template <typename T, typename Ptr> class UniquePtr<T[], Ptr> {
      public:
        using pointer = Ptr;
        using element_type = T;
        ...
        void reset(T *ptr = nullptr) noexcept {
            if (ptr_ != nullptr) {
                delete[] ptr_; // Use delete[] for arrays
            }
            ptr_ = ptr;
        }
