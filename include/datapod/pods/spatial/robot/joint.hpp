...
namespace datapod {
    ...
    struct Joint {
        ...
        auto members() noexcept { return std::tie(name, type, origin, axis, limits, parent, child); }
        auto members() const noexcept { return std::tie(name, type, origin, axis, limits, parent, child); }
        ...
        inline bool operator==(const Joint &other) const noexcept {
            return name == other.name && type == other.type && origin == other.origin && axis == other.axis &&
                   limits == other.limits && parent == other.parent && child == other.child;
        }
    ...
    namespace joint {
        inline Joint fixed(const String &name, const Pose &origin = pose::identity()) noexcept {
            return Joint{name,    Joint::Type::Fixed, origin,    Array<f64, 3>{1.0, 0.0, 0.0},
        ...
        inline Joint revolute(const String &name, const Array<f64, 3> &axis, const Joint::Limits &limits,
            ...
            return Joint{name, Joint::Type::Revolute, origin, axis, limits, kInvalidId, kInvalidId};
        }

        inline Joint continuous(const String &name, const Array<f64, 3> &axis,
                                const Pose &origin = pose::identity()) noexcept {
            return Joint{name, Joint::Type::Continuous, origin, axis, nullopt, kInvalidId, kInvalidId};
        }

        inline Joint prismatic(const String &name, const Array<f64, 3> &axis, const Joint::Limits &limits,
                               const Pose &origin = pose::identity()) noexcept {
            return Joint{name, Joint::Type::Prismatic, origin, axis, limits, kInvalidId, kInvalidId};
        }
    } // namespace joint
