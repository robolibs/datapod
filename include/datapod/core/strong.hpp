...
namespace std {
    ...
    template <typename T, typename Tag> class numeric_limits<datapod::Strong<T, Tag>> {
      public:
        static constexpr datapod::Strong<T, Tag> min() noexcept {
            return datapod::Strong<T, Tag>{std::numeric_limits<T>::min()};
        }
        static constexpr datapod::Strong<T, Tag> max() noexcept {
            return datapod::Strong<T, Tag>{std::numeric_limits<T>::max()};
        }
        static constexpr bool is_integer = std::is_integral_v<T>;
    };

    template <typename T, typename Tag> struct hash<datapod::Strong<T, Tag>> {
        size_t operator()(datapod::Strong<T, Tag> const &t) const { return hash<T>{}(t.v_); }
    };
