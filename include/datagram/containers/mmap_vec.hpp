...
namespace datagram {
    ...
    template <typename T, typename Key = std::uint32_t> struct BasicMmapVec {
        ...
        explicit BasicMmapVec(Mmap mmap)
            : mmap_{std::move(mmap)}, used_size_{static_cast<size_type>(mmap_.size() / sizeof(T))} {}
        ...
        template <typename... Args> T &emplace_back(Args &&...el) {
        ...
        T const *begin() const noexcept { return reinterpret_cast<T const *>(mmap_.data()); }
        T const *end() const noexcept { return begin() + used_size_; }
        T const *cbegin() const noexcept { return begin(); }
        T const *cend() const noexcept { return begin() + used_size_; }
        T *begin() noexcept { return reinterpret_cast<T *>(mmap_.data()); }
        T *end() noexcept { return begin() + used_size_; }

        friend T const *begin(BasicMmapVec const &a) noexcept { return a.begin(); }
        friend T const *end(BasicMmapVec const &a) noexcept { return a.end(); }

        friend T *begin(BasicMmapVec &a) noexcept { return a.begin(); }
        friend T *end(BasicMmapVec &a) noexcept { return a.end(); }
        ...
        template <typename It> void set(It begin_it, It end_it) {
            using diff_t = std::common_type_t<typename std::iterator_traits<It>::difference_type, size_type>;
            auto const range_size = std::distance(begin_it, end_it);
            verify(range_size >= 0 &&
                       static_cast<diff_t>(range_size) <= static_cast<diff_t>(std::numeric_limits<size_type>::max()),
                   "mmap_vec::set: invalid range");
        ...
        template <typename Arg> T *insert(T *it, Arg &&el) {
        ...
        template <class InputIt> T *insert(T *pos, InputIt first, InputIt last, std::input_iterator_tag) {
        ...
        template <class FwdIt> T *insert(T *pos, FwdIt first, FwdIt last, std::forward_iterator_tag) {
            if (empty()) {
                set(first, last);
                return begin();
            }
    ...
    template <typename T> using MmapVec = BasicMmapVec<T>;
