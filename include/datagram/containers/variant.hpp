...
namespace datagram {
    ...
    template <int N, typename... T> struct type_at_index;

    template <typename First, typename... Rest> struct type_at_index<0, First, Rest...> {
        using type = First;
    };

    template <int N, typename First, typename... Rest> struct type_at_index<N, First, Rest...> {
    ...
    template <typename... T> struct Variant {
        ...
        constexpr Variant(Arg &&arg) : idx_{static_cast<index_t>(index_of_type<Arg, T...>())} {
        ...
        template <typename F> auto apply(F &&f) -> decltype(f(std::declval<type_at_index_t<0U, T...> &>())) {
            if (idx_ == NO_VALUE) {
        ...
        template <typename F> auto apply(F &&f) const -> decltype(f(std::declval<type_at_index_t<0U, T...>>())) {
            if (idx_ == NO_VALUE) {
                throw_exception(std::runtime_error{"Variant::apply: no value"});
            }
            return apply(std::forward<F>(f), idx_, *this);
        ...
        static auto apply(F &&f, index_t const idx, Vs &&...vs)
            ...
            if constexpr (B + 15U < sizeof...(T)) {
                return apply<F, B + 16U>(std::forward<F>(f), idx, std::forward<Vs>(vs)...);
            }
        ...
        template <typename AsType> AsType &as() noexcept { return *reinterpret_cast<AsType *>(&storage_); }

        template <typename AsType> AsType const &as() const noexcept {
            return *reinterpret_cast<AsType const *>(&storage_);
        }

        hash_t hash_value() const noexcept {
            return apply([&](auto &&val) {
                auto const idx = index();
                auto h = hash_combine(BASE_HASH, idx);
                h = datagram::hash_value(val, h);
                return h;
            });
        }
    ...
    template <typename T, typename... Ts> bool holds_alternative(Variant<Ts...> const &v) noexcept {
        return v.idx_ == index_of_type<std::decay_t<T>, Ts...>();
    }

    template <std::size_t I, typename... Ts>
    constexpr datagram::type_at_index_t<I, Ts...> const &get(datagram::Variant<Ts...> const &v) noexcept {
        return v.template as<datagram::type_at_index_t<I, Ts...>>();
    }

    template <std::size_t I, typename... Ts>
    constexpr datagram::type_at_index_t<I, Ts...> &get(datagram::Variant<Ts...> &v) noexcept {
        return v.template as<datagram::type_at_index_t<I, Ts...>>();
    }

    template <class T, class... Ts> constexpr T const &get(datagram::Variant<Ts...> const &v) noexcept {
        static_assert(datagram::index_of_type<T, Ts...>() != datagram::TYPE_NOT_FOUND);
        return v.template as<T>();
    }

    template <class T, class... Ts> constexpr T &get(datagram::Variant<Ts...> &v) noexcept {
        static_assert(datagram::index_of_type<T, Ts...>() != datagram::TYPE_NOT_FOUND);
        return v.template as<T>();
    }

    template <class T, class... Ts> constexpr std::add_pointer_t<T> get_if(datagram::Variant<Ts...> &v) noexcept {
        static_assert(datagram::index_of_type<T, Ts...>() != datagram::TYPE_NOT_FOUND);
        return v.idx_ == datagram::index_of_type<T, Ts...>() ? &v.template as<T>() : nullptr;
    }

    template <std::size_t I, typename... Ts>
    constexpr std::add_pointer_t<datagram::type_at_index_t<I, Ts...> const>
    get_if(datagram::Variant<Ts...> const &v) noexcept {
        static_assert(I < sizeof...(Ts));
        return v.idx_ == I ? &v.template as<datagram::type_at_index_t<I, Ts...>>() : nullptr;
    }

    template <std::size_t I, typename... Ts>
    constexpr std::add_pointer_t<datagram::type_at_index_t<I, Ts...>> get_if(datagram::Variant<Ts...> &v) noexcept {
        static_assert(I < sizeof...(Ts));
        return v.idx_ == I ? &v.template as<datagram::type_at_index_t<I, Ts...>>() : nullptr;
    }

    template <class T, class... Ts>
    constexpr std::add_pointer_t<T const> get_if(datagram::Variant<Ts...> const &v) noexcept {
        static_assert(datagram::index_of_type<T, Ts...>() != datagram::TYPE_NOT_FOUND);
        return v.idx_ == datagram::index_of_type<T, Ts...>() ? &v.template as<T>() : nullptr;
    }
...
namespace std {
    ...
    template <typename Visitor, typename... T> constexpr auto visit(Visitor &&vis, datagram::Variant<T...> &&v) {
        return v.apply(std::forward<Visitor>(vis));
    }

    template <typename Visitor, typename... T> constexpr auto visit(Visitor &&vis, datagram::Variant<T...> const &v) {
        return v.apply(vis);
    }
