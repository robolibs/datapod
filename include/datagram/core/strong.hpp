...
namespace std {
    ...
    template <typename T, typename Tag> class numeric_limits<datagram::Strong<T, Tag>> {
      public:
        static constexpr datagram::Strong<T, Tag> min() noexcept {
            return datagram::Strong<T, Tag>{std::numeric_limits<T>::min()};
        }
        static constexpr datagram::Strong<T, Tag> max() noexcept {
            return datagram::Strong<T, Tag>{std::numeric_limits<T>::max()};
        }
        static constexpr bool is_integer = std::is_integral_v<T>;
    };

    template <typename T, typename Tag> struct hash<datagram::Strong<T, Tag>> {
        size_t operator()(datagram::Strong<T, Tag> const &t) const { return hash<T>{}(t.v_); }
    };
