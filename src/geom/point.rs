...
/// Hashable alias for `HashMap<PointKey, V>`.
pub type PointMap<V> = HashMap<PointKey, V>;
/// Hashable alias for `HashSet<PointKey>`.
pub type PointSet = HashSet<PointKey>;
...
/// Bit-exact hashable key for [`Point`]. Lets you use a Point as a map key
/// (raw `f64` is not `Eq`).
#[datapod::datapod]
...
impl Point {
    ...
    pub fn from_mat(value: [f64; 3]) -> Self {
